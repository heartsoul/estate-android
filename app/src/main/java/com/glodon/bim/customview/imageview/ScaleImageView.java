package com.glodon.bim.customview.imageview;
...
public class ScaleImageView extends ImageView implements
	...
	private int borderlength_x;
	private int borderlength_y;
	...
	private class MultiGestureDetector extends
		...
		public boolean onScale(ScaleGestureDetector detector) {
			...
			if (drawable != null) {
				suppMatrix.postScale(scaleFactor, scaleFactor,
	...
	private class AnimatedZoomRunnable implements Runnable {
		...
		public void run() {
			...
			if (((deltaScale > 1f) && (currentScale < targetZoom))
				...
				// again
				postOnAnimation(ScaleImageView.this, this);
	...
	private void checkMatrixBounds() {
		...
		if (rect.top > (viewHeight - borderlength_y) / 2) {
			deltaY = (viewHeight - borderlength_y) / 2 - rect.top;
		}
		if (rect.bottom < (viewHeight + borderlength_y) / 2) {
			deltaY = (viewHeight + borderlength_y) / 2 - rect.bottom;
		}
		if (rect.left > (viewWidth - borderlength_x) / 2) {
			deltaX = (viewWidth - borderlength_x) / 2 - rect.left;
		}
		if (rect.right < (viewWidth + borderlength_x) / 2) {
			deltaX = (viewWidth + borderlength_x) / 2 - rect.right;
		}
